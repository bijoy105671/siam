...
const REQUIRED_STORAGE_KEY = 'siam_security_otp_required';

const notifySecurityVerified = () => {
  const stamp = String(Date.now());
  try {
    localStorage.setItem(VERIFICATION_STORAGE_KEY, stamp);
    localStorage.removeItem(REQUIRED_STORAGE_KEY);
  } catch {}
...
export const SecurityOtpModal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const onStorage = (event: StorageEvent) => {
      ...
      if (event.key === REQUIRED_STORAGE_KEY && event.newValue) {
        try {
          const data = JSON.parse(event.newValue);
          show(new CustomEvent('siam:security-otp-required', { detail: { message: data?.message } }));
        } catch {}
      }
    };
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-4 bg-gradient-to-r from-emerald-600 to-sky-600 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/15 flex items-center justify-center"><ShieldCheck className="w-5 h-5" /></div>
            <div><div className="font-bold text-sm">Security Verification</div><div className="text-[11px] text-white/80">Admin confirmation required</div></div>
          </div>
          <button type="button" onClick={() => setOpen(false)} className="p-1.5 rounded-lg hover:bg-white/10" aria-label="Close"><X className="w-4 h-4" /></button>
        </div>
          ...
          {success ? (
            ...
            <>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setOpen(false)} disabled={busy} className="flex-1 px-4 py-2.5 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl">Cancel</button>
